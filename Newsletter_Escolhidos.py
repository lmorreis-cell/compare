...
def avaliar_risco_mercado():
    # Trocámos o ^V2TX (europeu falhado) pelo ^VIX (global resiliente)
    tickers = ["^VIX", "^STOXX", "^STOXX50E", "^TNX", "^IRX"]
    # yf.download agora com o parâmetro show_errors=False para silenciar lixo no terminal
    
    dados = yf.download(tickers, period="1y", progress=False, threads=False)['Close'].ffill().dropna(how='all')
    
    # Avaliação do VIX Global
    vix_atual = float(dados['^VIX'].dropna().iloc[-1]) if '^VIX' in dados and not dados['^VIX'].dropna().empty else 20.0
    vix_m20 = dados['^VIX'].rolling(window=20).mean().iloc[-1] if '^VIX' in dados else 20.0
    alerta_vix = vix_atual > vix_m20

    # Avaliação do STOXX 600
    spy_atual = float(dados['^STOXX'].dropna().iloc[-1]) if '^STOXX' in dados and not dados['^STOXX'].dropna().empty else 1.0
    spy_m50 = dados['^STOXX'].rolling(window=50).mean().iloc[-1] if '^STOXX' in dados else 1.0
    mercado_em_alta = spy_atual > spy_m50

    # Atrito de Amplitude (Europa Geral vs Mega-Caps Europeias)
    try:
        spy_perf_20d = (spy_atual / dados['^STOXX'].iloc[-20]) - 1
        rsp_perf_20d = (dados['^STOXX50E'].iloc[-1] / dados['^STOXX50E'].iloc[-20]) - 1
        amplitude_fraca = (rsp_perf_20d - spy_perf_20d) > 0.015
    except:
        amplitude_fraca = False

    # Mantemos a Curva de Juros Americana como a "mãe" da liquidez ocidental
    t10 = float(dados['^TNX'].dropna().iloc[-1]) if '^TNX' in dados and not dados['^TNX'].dropna().empty else 0.0
    t3m = float(dados['^IRX'].dropna().iloc[-1]) if '^IRX' in dados and not dados['^IRX'].dropna().empty else 0.0
    spread_curva = t10 - t3m
    curva_invertida = spread_curva < 0

    pontos_risco = sum([alerta_vix, not mercado_em_alta, amplitude_fraca, curva_invertida])
    
    if pontos_risco == 0: regime, cor, aviso = "CONSTRUTIVO", "#3fbf8f", "Mercado Europeu saudável. Forte expansão."
    elif pontos_risco == 1: regime, cor, aviso = "NEUTRO", "#f0b90b", "Atrito detetado na estrutura europeia ou macro global."
    elif pontos_risco == 2: regime, cor, aviso = "CAUTELA", "#f28b24", "Risco elevado. VIX agitado ou liquidez sob stress."
    else: regime, cor, aviso = "DEFENSIVO", "#e06a5a", "PERIGO MACRO. Liquidez asfixiada e capitulação europeia iminente."

    nlg_risco = "O VIX Global dita o pânico institucional. A posição face à M50 do STOXX 600 dita a tendência primária europeia. A Amplitude mede se a subida é alargada ou mascarada por 50 mega-caps (Euro Stoxx 50). A Curva de Juros Americana é mantida no painel porque a Reserva Federal dita a liquidez macroeconómica de todo o ocidente."
    ...
    return {
        ...
        "curva_status": "INVERTIDA" if curva_invertida else "Normal",
        "curva_cor": "var(--vermelho)" if curva_invertida else "var(--verde)",
        "nlg": nlg_risco
    }

def avaliar_breadth_macro():
    """Analisa os 11 Setores do STOXX 600 (ETFs Oficiais da iShares na Alemanha)"""
    try:
        etfs = {"Tecnologia": "EXV3.DE", "Saúde": "EXV4.DE", "Finanças": "EXH2.DE", 
                "Indústria": "EXH4.DE", "Comunicações": "EXV8.DE", "Banca": "EXV1.DE", 
                "Bens Pessoais": "EXH7.DE", "Materiais": "EXV6.DE", "Utilities": "EXV9.DE", 
                "Imobiliário": "EXI5.DE", "Alimentação": "EXH3.DE"}
        ...
        for ticker in tickers:
            if dados[ticker].iloc[-1] > dados[ticker].rolling(window=200).mean().iloc[-1]:
                setores_alta += 1
                
        breadth_pct = (setores_alta / len(tickers)) * 100
        status = "Forte" if breadth_pct >= 70 else "Misto" if breadth_pct >= 40 else "Fraco"
        cor = "var(--verde)" if breadth_pct >= 70 else "var(--amarelo)" if breadth_pct >= 40 else "var(--vermelho)"
        
        return {"pct": f"{breadth_pct:.0f}%", "status": status, "cor": cor, "largura": f"{breadth_pct}%", "num": setores_alta, "total": len(tickers)}
    except:
...
def avaliar_curva_juros():
    """Lê o spread entre a yield do Tesouro a 10 Anos e a 3 Meses com proteção Anti-NaN"""
    try:
        dados = yf.download(["^TNX", "^IRX"], period="5d", progress=False, threads=False)['Close']
        
        # O dropna() garante que vai buscar o último valor que é de facto um número real
        t10 = float(dados['^TNX'].dropna().iloc[-1]) if not dados['^TNX'].dropna().empty else 0.0
        t3m = float(dados['^IRX'].dropna().iloc[-1]) if not dados['^IRX'].dropna().empty else 0.0
        
        if t10 == 0.0 or t3m == 0.0:
             return {"status": "Erro API (Sem Dados)", "cor": "var(--mudo)", "valor": "0.00"}
...
def gerar_relatorio_excecao(fechos, volumes, tickers):
    """
    Motor de Triagem Diária - Calibrado para o STOXX 600 (Europa)
    Inclui bloqueio de ruído de liquidez (Capital Transacionado > 5M).
    """
    excecoes = []
    
    # Índices a ignorar na varredura de ações individuais
    indices_macro = ['^STOXX', '^STOXX50E', '^V2TX', '^TNX', '^IRX']
    
    for ticker in tickers:
        try:
            if ticker not in fechos.columns or ticker in indices_macro:
                continue
            ...
            # --- O FILTRO DE OURO EUROPEU (ANTI-RUÍDO) ---
            # Exige que a ação movimente, em média, mais de 5 Milhões de EUR/GBP/CHF por dia.
            # Se movimentar menos, ignoramos os "falsos" picos de volume de fundos pequenos.
            capital_transacionado = vol_m20 * preco_hoje
            if capital_transacionado < 5_000_000:
                continue

            # --- CÁLCULOS TÉCNICOS ---
            # Cálculo RSI (Wilder's EMA)
            delta = hist.diff()
            gain = delta.where(delta > 0, 0).ewm(alpha=1/14, adjust=False).mean()
            loss = (-delta.where(delta < 0, 0)).ewm(alpha=1/14, adjust=False).mean()
            rs = gain / loss
            rsi_hoje = 100 - (100 / (1 + rs)).iloc[-1]
            ...
            if vol_hoje > (vol_m20 * 2.5) and vol_m20 > 0:
                ...
                excecoes.append({
                    ...
                    "tipo": "Fuga de Volume", 
                    "cor": cor,
                    "desc": f"Volume {vol_hoje/vol_m20:.1f}x superior à média mensal. Forte {direcao} na praça europeia ({var_diaria:+.1f}%).",
                    "impacto": impacto + 5 
                })
            
            # 2. VERDADEIRO Golden / Death Cross 
            if m50_ontem < m200_ontem and m50_hoje >= m200_hoje:
                excecoes.append({
                    "ticker": ticker, 
                    "tipo": "Golden Cross", 
                    "cor": "var(--verde)",
                    "desc": "Ignição Macro: M50 cruzou hoje acima da M200. Confirmação técnica de bull market europeu.",
                    "impacto": 100 
                })
                ...
                excecoes.append({
                    ...
                    "tipo": "Death Cross", 
                    "cor": "var(--vermelho)",
                    "desc": "Falência Estrutural: M50 cruzou abaixo da M200. Risco elevado de distribuição institucional na Europa.",
                    "impacto": 100 
                })
                
            # 3. Quebra de Suporte / Recuperação de Resistência (M200)
            ...
            # 4. Anomalia de Volatilidade Extrema (Cortado de 7% para 6% para refletir o menor Beta europeu)
            elif abs(var_diaria) > 6.0:
                cor = "var(--verde)" if var_diaria > 0 else "var(--vermelho)"
                excecoes.append({
                    "ticker": ticker,
                    "tipo": "Choque de Volatilidade",
                    "cor": cor,
                    "desc": f"Movimento parabólico na sessão ({var_diaria:+.1f}%). Numa large-cap europeia, indica fortíssimo choque de catalisadores.",
                    "impacto": impacto
                })
            ...
            if rsi_hoje > 80:
                excecoes.append({
                    ...
                    "cor": "var(--amarelo)",
                    "desc": f"RSI parabólico ({rsi_hoje:.1f}). O ativo está num esticão insustentável a curto prazo. Risco de pullback.",
                    "impacto": impacto + 3
                })
            elif rsi_hoje < 20:
                 excecoes.append({
    ...
    # Ordenar por impacto e filtrar únicos
    ...
    # Extrair os Nomes para os top 12 alertas
...
def gerar_grafico_backtest(fechos):
    """Walk-Forward Backtest Real com Rotação Dinâmica Mensal (Motor Europeu)"""
    ...
    try:
        ...
        # SUBSTITUIÇÃO DO 'SPY' PELO BENCKMARK EUROPEU '^STOXX'
        if len(fechos) < 250 or '^STOXX' not in fechos.columns:
        ...
        tickers_validos = [c for c in fechos_limpos.columns if c not in ['^STOXX', '^VIX', '^TNX', '^IRX']]
        ...
        # Isola o STOXX 600 exato no mesmo calendário para a comparação ser justa
        retornos_idx = fechos_limpos['^STOXX'].pct_change().fillna(0).loc[serie_final.index]
        eq_idx = (1 + retornos_idx).cumprod() * 100
        
        # Cálculo das métricas finais (mantendo as 2 casas decimais)
        alfa = (eq_estr.iloc[-1] - 100) - (eq_idx.iloc[-1] - 100)
        retorno_estr = eq_estr.iloc[-1] - 100
        retorno_idx = eq_idx.iloc[-1] - 100
        
        nlg_calc = nlg_base + f"O motor avalia os ativos a cada 21 dias, corta os que perdem força e compra os novos líderes (Rebalanceamento Mensal). Alfa gerado: <strong style='color: {'var(--verde)' if alfa > 0 else 'var(--vermelho)'}'>{alfa:+.2f}%</strong> face ao STOXX 600."
        ...
        plt.plot(eq_idx.index, eq_idx.values, color='#8a94a8', 
                 label=f'STOXX 600 ({retorno_idx:+.2f}%)', linewidth=1.2, linestyle='-')
...
def calcular_position_sizing(a_comp):
    ...
    volatilidade_alvo = 15.0  # Volatilidade de referência de um mercado saudável (ex: STOXX 600)
...
def auditar_fundamentais(ticker):
    try:
        ...
        # --- CAÇA AOS FLUXOS OCULTOS (REDE DE ARRASTÃO YFINANCE) ---
        
        # 1. Tentar todas as chaves possíveis para Shorts
        short_pct = info.get('shortPercentOfFloat') or info.get('sharesPercentSharesOut')
        if not short_pct:
            s_short = info.get('sharesShort')
            s_out = info.get('floatShares') or info.get('sharesOutstanding') or info.get('impliedSharesOutstanding')
            if s_short and s_out and float(s_out) > 0:
                short_pct = float(s_short) / float(s_out)
            else:
                short_pct = 0
                
        # 2. Tentar todas as chaves possíveis para Insiders
        insider_pct = info.get('heldPercentInsiders') or info.get('insiderHoldings') or info.get('insiderPercent') or 0   
...
def executar_screener():
    # Lê APENAS as ações europeias
    with open("tickers_escolhidos_europe.txt", "r") as f:
        tickers = [linha.strip().upper() for linha in f if linha.strip()]
        
    # Injetar o Benchmark Europeu
    tickers_globais = list(set(tickers + ['^STOXX']))
    ...
    # Todos os ativos da lista de escolhidos são avaliados e mapeados nos Tiers
    ...
    tier_a, tier_b, quarentena, blacklist, dados_plot = [], [], [], [], []

    # === O LOOP MANTÉM-SE EXATAMENTE IGUAL DAQUI PARA A FRENTE ===
...
def formatar_tabela(acoes):
    ...
    # --- CÁLCULO DINÂMICO DOS EARNINGS ---
        ...
        formatadas.append({
            ...
            # ADICIONA ESTA LINHA AQUI NO MEIO:
            "tier_eu": a.get('tier_europeu', 'Quarentena'),
...
<head>
<meta charset="utf-8">
<title>Radar Quantitativo Europeu - {{ data }}</title>

<!-- METADADOS OPEN GRAPH PARA DISCORD/REDES SOCIAIS a imagem fica neste bloco-->
<meta property="og:title" content="Radar Europeu - Escolha Discord" {{ data }}/>
...
.badge-alerta-vermelho {
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 15px;">
        <div>
            <h1 style="margin: 0; font-size: 26px; color: #fff; letter-spacing: -0.5px; text-transform: uppercase;">Radar Quantitativo Europeu - Seleção do Discord</h1>
      ...
      <div class="painel-destaque">
          <div class="card-risco">
              ...
              <div class="metricas">
                  ...
                  <span>STOXX 600: <strong>{{ risco.spy }}</strong></span>
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
              <span class="dica-edu dica-desce" style="font-weight: bold; font-size: 12px; color: var(--mudo); border-bottom: 1px dotted var(--mudo); cursor: help;">
                  ...
                  <span class="dica-texto" style="width: 350px; font-weight: normal; text-transform: none;">
                      ...
                      O algoritmo audita a força dos 11 ETFs Setoriais do STOXX 600 (Tecnologia, Saúde, Energia, etc).<br><br>
...
def funcao_legado_remover_se_desnecessario():
    ...
    pass
    """
    html_saida = Template(newsletter_interativa.html).render(
    ...
    with open("newsletter_interativa.html", "w", encoding="utf-8") as f: f.write(html_saida)
...
if __name__ == "__main__":
    ...
    breadth_global = avaliar_breadth_macro() 
    ...
    # Monitoriza os verdadeiros índices do continente europeu
    dados_idx_raw = yf.download(["^STOXX", "^STOXX50E", "^GDAXI", "^FCHI", "PSI20.LS"], period="5d", progress=False, threads=False)['Close']
    indices_diarios = []
    for nome, t in [("STOXX 600", "^STOXX"), ("Euro Stoxx 50", "^STOXX50E"), ("DAX (Alemanha)", "^GDAXI"), ("CAC 40 (França)", "^FCHI"), ("PSI 20 (Lisboa)", "PSI20.LS")]:
        try:
    ...
    print("=== MODO NEWSLETTER EUROPA ===")
    
    # Recebemos apenas as 10 variáveis originais
    ...
    destaques_html = formatar_tabela(analisar_destaques_pedido(sys.argv[1])) if len(sys.argv) > 1 else []

    alocacao_total = sum([float(str(a.get('position_size', '0')).replace('%', '')) for a in t_a]) if t_a else 0
    dados_carteira = {"total_alocado": f"{alocacao_total:.1f}", "liquidez": f"{max(0, 100 - alocacao_total):.1f}"}

    hash_autoria = "RADAR-EU-" + hashlib.sha256(f"Luís Reis-{data_hoje}-{len(t_a)}".encode()).hexdigest()[:16].upper()
    ...
    html_final = Template(html_template).render(
        risco=risco_global, breadth=breadth_global, fluxo=smart_money, curva=curva_juros,          
        correlacao=correlacao_a, data=data_hoje, carteira=dados_carteira, screener=dados_screener,
        movers=movers_diarios, indices=indices_diarios, destaques=destaques_html,
        graf_setores=gerar_grafico_setores(t_a), excecoes=alertas_diarios,
        tier_a=formatar_tabela(t_a), tier_b=formatar_tabela(t_b),
        quarentena=formatar_tabela(t_q), blacklist=formatar_tabela(bl),
        g_disp=g_disp, graf_backtest=g_back, hash_autoria=hash_autoria,
        total_ativos = len(t_a) + len(t_b) + len(t_q) + len(bl),
        q_a = len(t_a), q_b = len(t_b), q_q = len(t_q), q_bl = len(bl),
        breadth_interno=dados_breadth, barras_sp500=breadth_macro,
        grafico_rrg=grafico_rrg, macro=datas_macro, universo=mapa_cobertura
    )
    
    sufixo_data = datetime.now().strftime("%d%b%y").lower()
    nome_ficheiro = f"radar_europeu_escolhidos_{sufixo_data}.html"
    ...
    print(f"=== SUCESSO: Newsletter Europeia gerada em {nome_ficheiro} ===")